...
//新的连接
void StarterUI::OnNewConnection(){

    L_DEBUG("StarterUI::OnNewConnection");
    newsocket = m_LocalServer->nextPendingConnection();
    connect(newsocket, SIGNAL(readyRead()), this, SLOT(OnReadyRead()));
}

// 读数据
void StarterUI::OnReadyRead(){

    L_DEBUG("StarterUI::OnReadyRead");
    ...
    // 读出数据
    readMsg = in.readAll();
    GParams::instance()->SetArgSave(readMsg);
    L_DEBUG(readMsg.toStdString());
    L_DEBUG("data is above");
    // QByteArray pkg;
    // pkg.append("you are beautiful");
    // newsocket->write(pkg);
...
void StarterUI::OnShotDone(Starter* starter)
{
    ...
    if (newsocket)
    {
        L_DEBUG("send message");
        QByteArray pkg;
        pkg.append(std::to_string(starter->m_exitCode).c_str());
        newsocket->write(pkg);
        newsocket->close();
        L_DEBUG("send pkg to {0}, data: {1}, size: {2}", newsocket->objectName().toInt(), pkg.toInt(), pkg.size())
    }else{
        L_DEBUG("no send  message");
    }
